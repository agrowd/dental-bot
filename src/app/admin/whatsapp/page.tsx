...
export default function WhatsAppPage() {
    ...
    const handleActivate = async (forceClean = false) => {
        setLoading(true);
        try {
            const res = await fetch(`${BOT_URL}/start`, {
                method: 'POST',
                ...
                <div className="card p-8">
                    ...
                    {status === 'disconnected' && (
                        <div className="text-center py-8">
                            ...
                            <p className="text-slate-600 mb-6">El bot está apagado o desconectado.</p>
                            <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
                                <button onClick={() => handleActivate(false)} disabled={loading} className="btn btn-primary">
                                    {loading ? 'Conectando...' : '🚀 Conectar Bot'}
                                </button>
                                <button onClick={() => handleActivate(true)} disabled={loading} className="btn btn-secondary text-purple-700 bg-purple-50 hover:bg-purple-100 border-purple-200">
                                    {loading ? 'Generando...' : '⚡ Generar Nuevo QR (Limpiar Sesión)'}
                                </button>
                            </div>
                            <p className="text-xs text-slate-400 mt-4">💡 Si desvinculaste WhatsApp desde tu celular, tocá <strong>"Generar Nuevo QR"</strong> para obtener un código de escaneo limpio.</p>
                        </div>
                    )}
                            ...
                            {qrCode ? (
                                <>
                                    ...
                                    <div className="flex items-center justify-center gap-3 mt-4">
                                        ...
                                        <button onClick={() => handleActivate(true)} className="btn btn-secondary text-purple-700 bg-purple-50">
                                            ⚡ Re-generar QR
                                        </button>
                                    </div>
                                </>
                                ...
                                <div>
                                    <p className="text-yellow-600 font-medium mb-4">Iniciando conexión WhatsApp...</p>
                                    <p className="text-xs text-slate-500 mb-6">Si no aparece el código QR en unos segundos, tocá el botón para forzar uno nuevo.</p>
                                    <button onClick={() => handleActivate(true)} disabled={loading} className="btn btn-secondary text-purple-700 bg-purple-50">
                                        ⚡ Forzar Nuevo Código QR
                                    </button>
                                </div>
                            )}
                        </div>
