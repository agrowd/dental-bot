...
export default function AdminLayout({ children }: { children: ReactNode }) {
    ...
    // Poll attention count every 30 seconds
    useEffect(() => {
        const fetchCount = async () => {
            try {
                const res = await fetch('/api/attention-count');
            ...
            <aside className="w-64 bg-white border-r border-[var(--border)] flex flex-col">
                ...
                <div className="p-6 border-b border-[var(--border)]">
                    <Link href="/admin" className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-gradient-to-r from-blue-600 to-blue-700 rounded-xl flex items-center justify-center shadow-lg shadow-blue-500/20">
                            <svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" />
                            </svg>
                        </div>
                        <div>
                            <h1 className="font-bold text-slate-900">OdontoBot</h1>
                            <p className="text-xs text-slate-500">Panel Admin</p>
                        </div>
                    </Link>
                </div>

                {/* 🚨 Attention Alert Banner — only shows when count > 0 */}
                {attentionCount > 0 && (
                    <Link
                        href="/admin/conversations?filter=attention"
                        className="mx-3 mt-3 flex items-center gap-2.5 px-3 py-2.5 rounded-xl bg-red-50 border border-red-200 hover:bg-red-100 transition-colors"
                    >
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[10px] text-red-500 leading-tight">Clic para revisar →</p>
                        </div>
                    </Link>
                )}
                    ...
                    {navItems.map((item) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                {isConversations && attentionCount > 0 && (
