...
async function seed() {
    try {
        console.log("Connecting to MongoDB for V5 seed...");
        ...
        if (existing) {
            existing.published = { entryStepId: "welcome", steps: steps, fallbackMessage: "No entiendo esa opción. Por favor elegí una válida o escribí M para volver al menú." };
            existing.draft = existing.published;
            existing.publishedVersion = (existing.publishedVersion || 0) + 1;
            existing.updatedAt = new Date();
            await existing.save();
            console.log(`✅ Flow '${flowName}' updated to V${existing.publishedVersion} (Clean Mode)`);
        }
        process.exit(0);
    } catch (e) {
        console.error(e);
